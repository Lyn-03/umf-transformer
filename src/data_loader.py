...
# ---------------------------------------------------------------------------
# Unimodal Datasets for Separate Face / Fingerprint Corpora
# ---------------------------------------------------------------------------

class UnimodalFaceDataset(Dataset):
    """Unimodal face dataset for CASIA-WebFace extracted format.

    Expects a directory structure where each subdirectory is a subject
    containing face images::

        data/casia-webface-extracted/
            00000/
                000.jpg, 001.jpg, ...
            00001/
                000.jpg, 001.jpg, ...
    ...
    Returns tuples of ``(face_tensor, subject_id, quality)``.

    Args:
        root_dir: Path to the extracted CASIA-WebFace directory.
        image_size: Target square image size. Defaults to 224.
        is_training: Whether to apply training augmentations. Defaults to True.
        apply_quality_degradation: Whether to simulate quality degradation.
            Defaults to True.
        limit_subjects: Optional cap on number of subjects to load.
    """
    ...
    def __init__(
        ...
        if not self.root_dir.exists():
            raise FileNotFoundError(f"Face dataset root not found: {self.root_dir}")

        self.transform = get_face_transforms(
        ...
        self.subject_dirs: List[str] = sorted(
            d.name for d in self.root_dir.iterdir()
            if d.is_dir() and not d.name.startswith(".")
        )
        if limit_subjects is not None:
            self.subject_dirs = self.subject_dirs[:limit_subjects]

        if not self.subject_dirs:
            raise ValueError(f"No subject directories found in {self.root_dir}")

        self.subject_to_id = {name: idx for idx, name in enumerate(self.subject_dirs)}
        ...
        for subj in self.subject_dirs:
            ...
            for ext in (".jpg", ".jpeg", ".png", ".bmp"):
                for img_path in sorted(subj_dir.glob(f"*{ext}")):
                    self.samples.append((img_path, sid))

        if not self.samples:
            raise ValueError(f"No face images found in {self.root_dir}")
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int, float]:
        ...
        quality = self.quality_estimator.estimate(tensor)
        return tensor, subject_id, quality

    def get_num_subjects(self) -> int:
        return len(self.subject_dirs)


class UnimodalFingerprintDataset(Dataset):
    """Unimodal fingerprint dataset for SOCOFing format.

    Expects SOCOFing Real/ directory with naming convention::

        {ID}__{M/F}_Left/Right_{finger}_finger.BMP

    Examples: ``100__M_Left_index_finger.BMP``, ``100__M_Right_thumb_finger.BMP``.
    The subject ID is extracted from the portion before ``__``.

    Returns tuples of ``(fingerprint_tensor, subject_id, quality)``.

    Args:
        root_dir: Path to the SOCOFing/Real directory.
        image_size: Target square image size. Defaults to 224.
        is_training: Whether to apply training augmentations. Defaults to True.
        apply_quality_degradation: Whether to simulate quality degradation.
            Defaults to True.
    ...
    def __init__(
        ...
        if not self.root_dir.exists():
            raise FileNotFoundError(f"Fingerprint dataset root not found: {self.root_dir}")

        self.transform = get_fingerprint_transforms(
            image_size=image_size,
            is_training=is_training,
            apply_quality_degradation=apply_quality_degradation,
        )
        ...
        for ext in (".bmp", ".BMP", ".jpg", ".jpeg", ".png"):
            for img_path in sorted(self.root_dir.glob(f"*{ext}")):
                filename = img_path.stem
                if "__" not in filename:
                    continue
                raw_id = filename.split("__")[0]
                if raw_id not in subject_ids:
                    subject_ids[raw_id] = len(subject_ids)
                sid = subject_ids[raw_id]
                self.samples.append((img_path, sid))

        if limit_subjects is not None:
            allowed = set(list(subject_ids.keys())[:limit_subjects])
            self.samples = [(p, sid) for p, sid in self.samples
                            if list(subject_ids.keys())[sid] in allowed]
            subject_ids = {k: i for i, k in enumerate(allowed)}

        self.subject_ids = subject_ids

        if not self.samples:
            raise ValueError(f"No fingerprint images found in {self.root_dir}")
...
class SyntheticPairDataset(Dataset):
    """Synthetic cross-modal pair dataset for Phase 2 joint training.

    Randomly pairs a face sample from ``UnimodalFaceDataset`` with a
    fingerprint sample from ``UnimodalFingerprintDataset``.  Since the
    two corpora have **no shared identities**, every pair is inherently
    cross-subject (impostor-like).  The fusion transformer learns to
    produce a unified embedding despite the modality gap.

    At each ``__getitem__`` call, a face and a fingerprint are sampled
    uniformly at random from their respective datasets.

    Args:
        face_dataset: An ``UnimodalFaceDataset`` instance.
        fingerprint_dataset: An ``UnimodalFingerprintDataset`` instance.
        samples_per_epoch: Number of synthetic pairs per epoch. If None,
            defaults to ``max(len(face_dataset), len(fp_dataset))``.

    Returns:
        Tuple of ``(face, fingerprint, face_subject_id, fp_subject_id,
        face_quality, fp_quality)``.
    """

    def __init__(
        self,
        face_dataset: UnimodalFaceDataset,
        fingerprint_dataset: UnimodalFingerprintDataset,
        ...
        self.samples_per_epoch = samples_per_epoch or max(
            len(face_dataset), len(fingerprint_dataset)
        )
    ...
    def __getitem__(
        ...
        face_idx = random.randint(0, len(self.face_dataset) - 1)
        fp_idx = random.randint(0, len(self.fingerprint_dataset) - 1)

        face_tensor, face_sid, face_q = self.face_dataset[face_idx]
        fp_tensor, fp_sid, fp_q = self.fingerprint_dataset[fp_idx]

        return face_tensor, fp_tensor, face_sid, fp_sid, face_q, fp_q


# ---------------------------------------------------------------------------
# DataLoader Factory (original, for backward compatibility)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# DataLoader Factory for Separate Datasets (CASIA-WebFace + SOCOFing)
# ---------------------------------------------------------------------------

def get_dataloaders_separate(
    face_path: str,
    ...
    pin_memory: bool = True,
    apply_quality_degradation: bool = True,
    limit_face_subjects: Optional[int] = None,
    limit_fp_subjects: Optional[int] = None,
    synthetic_samples_per_epoch: Optional[int] = None,
    distributed: bool = False,
) -> Dict[str, Any]:
    """Create DataLoaders for separate face and fingerprint datasets.

    Builds unimodal dataloaders for Phase 1 (independent encoder pre-training)
    and a synthetic-pair dataloader for Phase 2 (joint fusion training).

    Phase 1 returns two separate loaders:
        - ``face_loader``: yields ``(face_tensor, subject_id, quality)``
        - ``fp_loader``: yields ``(fingerprint_tensor, subject_id, quality)``

    Phase 2 returns a single loader:
        - ``pair_loader``: yields ``(face, fingerprint, face_sid, fp_sid,
          face_quality, fp_quality)``

    Args:
        face_path: Root directory for face images (CASIA-WebFace extracted).
        fingerprint_path: Root directory for fingerprint images (SOCOFing/Real).
        batch_size: Batch size for all loaders. Defaults to 64.
        num_workers: DataLoader worker processes. Defaults to 4.
        image_size: Target square image size. Defaults to 224.
        seed: Random seed for reproducibility. Defaults to 42.
        pin_memory: Pin CPU memory for faster GPU transfer. Defaults to True.
        apply_quality_degradation: Simulate quality degradation. Defaults to True.
        limit_face_subjects: Cap on face subjects (None = all).
        limit_fp_subjects: Cap on fingerprint subjects (None = all).
        synthetic_samples_per_epoch: Number of synthetic pairs per epoch
            for Phase 2. Defaults to max of both dataset lengths.

    Returns:
        Dictionary with keys:
            - ``face_loader``: Phase 1 face DataLoader
            - ``fp_loader``: Phase 1 fingerprint DataLoader
            - ``pair_loader``: Phase 2 synthetic-pair DataLoader
            - ``face_dataset``: UnimodalFaceDataset instance
            - ``fp_dataset``: UnimodalFingerprintDataset instance
            - ``pair_dataset``: SyntheticPairDataset instance
            - ``num_face_subjects``: Number of face subjects
            - ``num_fp_subjects``: Number of fingerprint subjects
    """
    set_seed(seed)

    should_pin = pin_memory and torch.cuda.is_available()

    # Helper: build a DataLoader, optionally with a DistributedSampler
    ...
    # Phase 1: unimodal datasets
    face_dataset = UnimodalFaceDataset(
        root_dir=face_path,
        image_size=image_size,
        is_training=True,
        apply_quality_degradation=apply_quality_degradation,
        limit_subjects=limit_face_subjects,
    )
    fp_dataset = UnimodalFingerprintDataset(
        root_dir=fingerprint_path,
        image_size=image_size,
        is_training=True,
        apply_quality_degradation=apply_quality_degradation,
        limit_subjects=limit_fp_subjects,
    )

    face_loader = _make_loader(face_dataset)
    fp_loader = _make_loader(fp_dataset)

    # Phase 2: synthetic cross-modal pairs
    pair_dataset = SyntheticPairDataset(
        face_dataset=face_dataset,
        fingerprint_dataset=fp_dataset,
        samples_per_epoch=synthetic_samples_per_epoch,
    )
    pair_loader = _make_loader(pair_dataset)
    ...
    return {
        ...
        "num_face_subjects": face_dataset.get_num_subjects(),
        "num_fp_subjects": fp_dataset.get_num_subjects(),
    }
