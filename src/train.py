...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        elif "fused_embedding" in outputs:
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Validate", leave=False, dynamic_ncols=True):
            ...
            if "fused_embedding" in outputs:
                ...
                emb = torch.cat(
                    [outputs["face_embedding"], outputs["fingerprint_embedding"]],
                    dim=1,
                )
    ...
    for epoch in range(1, config.epochs_phase1 + 1):
        ...
        for batch in pbar:
            ...
            loss_face, face_dict = face_criterion(
                outputs["face_embedding"], labels
            )
            loss_fp, fp_dict = fp_criterion(
                outputs["fingerprint_embedding"], labels
            )
            ...
            face_correct += (face_criterion.arcface(
                F.normalize(outputs["face_embedding"], p=2, dim=1), labels
            ).argmax(dim=1) == labels).sum().item()
            face_total += labels.size(0)
            fp_correct += (fp_criterion.arcface(
                F.normalize(outputs["fingerprint_embedding"], p=2, dim=1), labels
            ).argmax(dim=1) == labels).sum().item()
...
def main(config: Optional[TrainConfig] = None) -> nn.Module:
    ...
    if config.resume_from is not None and Path(config.resume_from).exists():
        ...
        start_phase = phase
        start_epoch = epoch
        logger.info(f"Resumed at phase {phase}, epoch {epoch}")
